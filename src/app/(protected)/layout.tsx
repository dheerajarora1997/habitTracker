...
import { Activity, LayoutDashboard, CalendarDays, Users2, User, Shield, LogOut } from 'lucide-react';
...
export default function ProtectedLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-zinc-950 text-zinc-400">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-semibold tracking-wide text-zinc-300">Restoring secure session...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col bg-zinc-950 min-h-screen text-zinc-100 pb-12">
      {/* Global Glassmorphic Sticky Navbar */}
      <nav className="sticky top-0 z-40 w-full border-b border-zinc-900 bg-zinc-950/70 backdrop-blur-md">
          ...
          <div className="flex h-16 items-center justify-between">
            <div className="flex items-center gap-6">
              <Link href="/dashboard" className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold tracking-wider text-white hidden sm:block">
                  HABITPARTNER
                </span>
              </Link>
            ...
            {/* Profile Avatar / Phone & Logout Button */}
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full bg-indigo-900/40 border border-indigo-500/30 flex items-center justify-center font-bold text-indigo-300 text-xs shadow-inner">
              ...
              <button
                ...
                className="p-2 rounded-xl border border-zinc-800 bg-zinc-900/50 hover:bg-red-950/20 hover:border-red-900/30 text-zinc-400 hover:text-red-400 transition-all text-xs flex items-center justify-center"
                title="Log Out"
              >
        ...
        <div className="md:hidden border-t border-zinc-900/80 px-4 py-2 flex justify-around bg-zinc-950/90 w-full">
          {navLinks.map((link) => {
            const Icon = link.icon;
            const isActive = pathname === link.href;
            return (
